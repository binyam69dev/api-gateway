  ...
  new GoogleStrategy(
    {
      ...
      callbackURL: `${process.env.APP_URL}/auth/admin/google/callback`,
      scope: ["profile", "email"],
    },
...
app.post("/auth/login", authLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!user.password_hash) {
      return res
        .status(401)
        .json({ error: "Please login using Google, GitHub, or Facebook" });
    }
    ...
    res.cookie("devToken", accessToken, {
      ...cookieOptions,
      maxAge: 60 * 60 * 1000,
    });
...
app.get(
  ...
  passport.authenticate("developer-google", {
...
app.get(
  ...
  passport.authenticate("developer-github", {
    failureRedirect: "/developer-auth.html",
  }),
  (req, res) => {
    res.cookie("devToken", req.user.token, cookieOptions);
...
app.get(
  ...
  passport.authenticate("developer-facebook", {
    failureRedirect: "/developer-auth.html",
  }),
  (req, res) => {
    res.cookie("devToken", req.user.token, cookieOptions);
  ...
  try {
    ...
    if (!user.password_hash) {
      return res
        .status(400)
        .json({
          error: "This account uses social login. No password to change.",
        });
    }
